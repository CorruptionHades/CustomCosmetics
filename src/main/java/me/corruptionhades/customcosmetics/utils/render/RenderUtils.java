...
public class RenderUtils implements IMinecraftInstance {
    ...
    public static void drawRoundedOutline(DrawContext context, double x, double y, double x2, double y2, double radius, double thickness, Color outline, Color fill) {
        renderRoundedQuad(context, x, y, x2, y2, radius, 20, outline);
        renderRoundedQuad(context, x + thickness, y + thickness, x2 - thickness, y2 - thickness, radius, 20, fill);
    }
    ...
    public static void drawTexturedRectangle(DrawContext context, float x, float y, String path) {
        try {
            URL url = RenderUtils.class.getResource("/assets/customcosmetics/" + path);
    ...
    private static final HashMap<Identifier, BufferedImage> imageCache = new HashMap<>();
    ...
    public static void drawScaledTexturedRect(DrawContext context, float x, float y, float scale, String path) {
        ...
        try {
            ...
            if(!imageCache.containsKey(Identifier.of("customcosmetics", path))) {
                ...
                BufferedImage image = imageCache.get(id);
                width = image.getWidth();
                height = image.getHeight();
            }
